...
We also present here tables of the mean and standard deviation values of the time dependent and independent Gompertz parameters. Table \ref{tab:RL12} shows
...
Using Welch's ANOVA test, we calculated that $p<1\times 10^{-9}$ for all demographics, but also when split between male and female. In figure \ref{fig:ROC_MeanBP} and \ref{fig:ROC_FRS} present 

![ROC curves for the model that used mean systolic and diastolic blood pressure as covariates in the linear predictor, stratified by age group and the number of years the survival outcome was predicted since participant starting the survey. ](./Rmarkdown_Plots/ROC_MeanBPModel_CAx-EventType.png){#fig:ROC_MeanBP}
