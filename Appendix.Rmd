...
```{r digit_summary, include=TRUE, echo=FALSE,results='asis',message=FALSE,warning=FALSE}
# Printing the table using kable
    cat(kable(bp_last_digit,format="latex", escape = F,booktabs = T, digits=3,
              linesep = rep(c(rep("",5),"\\addlinespace"),4),
                  col.names=c("Place","Sys/Dias",2*(0:4)),caption = 'Summary data for BP end digits') %>%  kable_styling(latex_options = "hold_position") )
    cat('\n')
```
...
We begin by noting the standard deviations for measurements of individual subjects as given in the column 'Mean of SD' in Table \ref{tab:sd_summary}.
The column 'Prob all rep' gives the theoretical probability that two of the three measurements for a subject would have the same value, if the measurements were independent and normally distributed with the given standard deviation (adjusted for the rounding), and assuming that rounding to particular digits is done in proportion to the fractions listed in Table \ref{tab:digit_summary}.
...
```
```{r sd_summary, include=TRUE, echo=FALSE,results='asis',message=FALSE,warning=FALSE}
...
```{r examinerPlot3, include=TRUE, echo=FALSE,results='asis',message=FALSE,warning=FALSE,fig.cap="Number of subjects with 3 equal measurements by examiner, blocked by place and type. Red band shows 95% probability range. Vertical green dashed line shows expected fraction; blue dotted line shows observed fraction over all examiners."}
...
In figure 3, we show the fraction of subjects with three equal measurements, by examiner, blocked by place and type.
...
On the other hand, if there is a trend in the measurements, then the second is least likely to be the odd one out. In fact, what we observe is that it is the third measurement that is least likely to differ from the other two, while the first is most likely.

```{r ternaryTable, include=TRUE, echo=FALSE,results='asis',message=FALSE,warning=FALSE}
...
table1 %<>% pivot_wider(names_from = Which_Diff, values_from = Normalized,id_cols = c("BPplace","BPtype","Exam"),names_prefix = "V")%>%
  rbind(data.frame(BPplace=rep(c("Home","Clinic"),2),BPtype=rep(c("Systolic","Diastolic"),each=2),Exam='Centre',V1=1/3,V2=1/3,V3=1/3))

size_values <- setNames(rep(1.5, length(levels(table1$Exam))), levels(table1$Exam))
size_values["Centre"] <- 3

color_values <- setNames(c14, levels(table1$Exam))
...
ggtern(data=table1, aes(x=V1, y=V2, z=V3)) +
  geom_point(aes(col=Exam, size=Exam),alpha=.75) +
  theme_bw() + 
  facet_grid(BPplace~BPtype) +
...
```
In figure 4, we show a ternary plot of the normalized frequencies of the three possible positions of the measurement that is unique, with one point for each examiner, blocked by place and type.
We see an almost uniform bias toward the first and away from the third as the distinct measurement, but also substantial differences among the examiners.
The largest outlier is examiner 3001, who has a much higher proportion of subjects with the first measurement being the unique one in the Home measurements, for both systolic and diastolic BP.
