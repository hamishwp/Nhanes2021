...
#    s2k <- apply(bp_sim[[which_bp]])
    ...
    G[[which_bp]][[where_bp]] <- ggplot(tibble(VarianceOverBeta = s2k[s2k<4]), aes(x=VarianceOverBeta)) + 
      geom_histogram(alpha=0.2, position="identity", aes(y = ..density..), binwidth = .04) + xlab('Variance/beta') +
