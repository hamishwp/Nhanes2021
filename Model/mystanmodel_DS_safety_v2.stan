functions {
  ...
  */
  ...
  vector linear_predictor(matrix M, vector m1, matrix Delta, vector m2,
     ...
     (M[,2]     - m1[2])         * beta[5]  +
     (Delta[,2] - m2[2])         * beta[6]  +
     (tauis[,3] - mean_tauis[3]) * beta[7] +
     (tauis[,4] - mean_tauis[4]) * beta[8]);
...
transformed parameters {
  ...
  xhat_D[2] =  (log_sum_exp(abs_Delta_i[,2]*beta[6]) - log(N)) ./beta[6];
  ...
  linnie=linear_predictor(M_i, xhat_M, abs_Delta_i, xhat_D, tauis, xhat_t, beta);
