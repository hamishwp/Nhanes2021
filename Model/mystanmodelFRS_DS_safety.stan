functions {
  ...
  */
  ...
  vector linear_predictor(vector FRS, matrix Delta, vector m2,
...
transformed parameters {
  ...
  linnie=linear_predictor(FRS, abs_Delta_i, xhat_D, tauis, xhat_t, beta);
  
  // for(i in 1:N){
  // linnie[i]=FRS[i] .* beta[1]  +
  //   (abs_Delta_i[i,1] - xhat_D[1])  * beta[2]  +
  //   (tauis[i,1] - xhat_t[1]) * beta[3] +
  //   (tauis[i,2] - xhat_t[2]) * beta[4] +
  //   (abs_Delta_i[i,2] - xhat_D[2]) * beta[5]  +
  //   (tauis[i,3] - xhat_t[3]) * beta[6] +
  //   (tauis[i,4] - xhat_t[4]) * beta[7];
  //  }
