...
\end{align*}
Because of rounding, the observed $S^2_i$ will be too small. We approximate the true $S^2_i$ by adding $\frac{1}{3}$, the variance
...
#    s2k <- apply(bp_sim[[which_bp]])
    ...
    for ( where_bp in BP_place_names){
      ...
      s2k_sim <- apply(bp_sim[[where_bp]]$BP,1,var) /
                         params$Gamma[[which_bp]]['beta',where_bp]
      all_s2k %<>% c(sort(s2k))
...
all_s2k.tbl <- subset(tibble(Real = all_s2k, Simulated = all_s2k_sim, 
                      HC = rep(rep(BP_place_names,rep(N,2)),2),
                      DS = rep(BP_type_names,c(N,N)*2) ), 
