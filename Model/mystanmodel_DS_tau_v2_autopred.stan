functions {
  ...
  */
  ...
  vector linear_predictor(matrix M, vector m1, matrix Delta, vector m2,
     ...
     (M[,2]     - m1[2])         * beta[5]  +
     (Delta[,2] - m2[2])         * beta[6]  +
     (vterm[,3] - mean_vterm[3]) * beta[7] +
     (vterm[,4] - mean_vterm[4]) * beta[8]);
...
transformed parameters {
  ...
  xhat_v[1] =  (log_sum_exp(tauis[,1]*beta[3]) - log(N)) ./beta[3];
  xhat_v[2] =  (log_sum_exp(tauis[,2]*beta[4]) - log(N)) ./beta[4];
  xhat_v[3] =  (log_sum_exp(tauis[,3]*beta[7]) - log(N)) ./beta[7];
  xhat_v[4] =  (log_sum_exp(tauis[,4]*beta[8]) - log(N)) ./beta[8];
  
  linnie=linear_predictor(M_i, xhat_M, abs_Delta_i, xhat_D, tauis, xhat_v, beta);
