functions {
  ...
  */
  ...
  vector linear_predictor(vector FRS, vector m1, matrix Delta, vector m2,
                          matrix tauis, vector mean_tauis, vector beta){
...
transformed parameters {
  ...
  // xhat_M[1] =  (log_sum_exp(M_i[,1]*beta[1]) - log(N)) ./beta[1];
  // xhat_M[2] =  (log_sum_exp(M_i[,2]*beta[5]) - log(N)) ./beta[5];
  // 
  // xhat_D[1] =  (log_sum_exp(Delta_i[,1]*beta[2]) - log(N)) ./beta[2];
  // xhat_D[2] =  (log_sum_exp(Delta_i[,2]*beta[6]) - log(N)) ./beta[6];
  // 
  // xhat_t[1] =  (log_sum_exp(tauis[,1]*beta[3]) - log(N)) ./beta[3];
  // xhat_t[2] =  (log_sum_exp(tauis[,2]*beta[4]) - log(N)) ./beta[4];
  // xhat_t[3] =  (log_sum_exp(tauis[,3]*beta[7]) - log(N)) ./beta[7];
  // xhat_t[4] =  (log_sum_exp(tauis[,4]*beta[8]) - log(N)) ./beta[8];
  
  xhat_D[1] =  (log_sum_exp(abs_Delta_i[,1]*beta[2]) - log(N)) ./ beta[2];
  xhat_D[2] =  (log_sum_exp(abs_Delta_i[,2]*beta[5]) - log(N)) ./ beta[5];
  
  xhat_t[1] =  (log_sum_exp(tauis[,1]*beta[3]) - log(N)) ./ beta[3];
  xhat_t[2] =  (log_sum_exp(tauis[,2]*beta[4]) - log(N)) ./ beta[4];
  xhat_t[3] =  (log_sum_exp(tauis[,3]*beta[6]) - log(N)) ./ beta[6];
  xhat_t[4] =  (log_sum_exp(tauis[,4]*beta[7]) - log(N)) ./ beta[7];
  
  linnie=linear_predictor(FRS, xhat_M, abs_Delta_i, xhat_D, tauis, xhat_t, beta);
