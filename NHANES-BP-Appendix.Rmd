...
```{r iqrnormbeta_RL1, echo=F,results='asis'}
convnames<-data.frame(variable=c("M i S","M i D","D i S","D i D","FRS.1998",
                                 "sigma C S","sigma H S","sigma C D","sigma H D"),
                      Covariate=c("Systolic Mean", "Diastolic Mean",
                                  "Systolic $|\\Delta|$","Diastolic $|\\Delta|$",
                                  "FRS (1998)",
                                  "Systolic Clinic Stand Dev","Systolic Home Stand Dev",
                                  "Diastolic Clinic Stand Dev","Diastolic Home Stand Dev"))

cat(kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%
        filter(runnum==1)%>%slice(c(3,4,1,2,5:8))%>%
        left_join(convnames,by="variable")%>%dplyr::select(Covariate,beta,Range)%>%
        setNames(c("Covariate","Beta Normalised (IQR)","Range")),
        format="latex", 
        escape = F,
        booktabs = T, 
        label = "iqrnormbeta",
...
```{r iqrnormbeta_RL2, echo=F,results='asis'}
cat(kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%
        filter(runnum==2)%>%slice(c(3,4,1,2,5:8))%>%
        left_join(convnames,by="variable")%>%dplyr::select(Covariate,beta,Range)%>%
        setNames(c("Covariate","Beta Normalised (IQR)","Range")),
        format="latex", 
        escape = F,
...
```{r iqrnormbeta_RL3, echo=F,results='asis'}
cat(kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%
...
```{r iqrnormbeta_RL4, echo=F,results='asis'}
cat(kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%
...
cat(survival::coxph(survival::Surv(age, Time+age, eventCVDHrt) ~ FRS.1998 +
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
cat(survival::coxph(survival::Surv(age, Time+age, eventall) ~ FRS.1998 +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesFRS) %>%
  broom::tidy(conf.int = TRUE) %>%    # Include confidence intervals
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
cat(survival::coxph(survival::Surv(age, Time+age, eventCVDHrt) ~ M_i_S + M_i_D +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
cat(survival::coxph(survival::Surv(age, Time+age, eventall) ~ M_i_S + M_i_D +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesA) %>%
  broom::tidy(conf.int = TRUE) %>%    # Include confidence intervals
