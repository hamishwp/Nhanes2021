...
cat(kable(read_csv(file="Results/beta_all_IQRnorm.csv", show_col_types = FALSE) %>%
        ...
        dplyr::select(Covariate, beta, Range) %>%
        setNames(c("Covariate", "Beta Normalised (IQR)", "Range")),
        format = "latex", 
        escape = FALSE,  # Ensure LaTeX commands in Covariate are not escaped
        booktabs = TRUE, 
        label = "iqrnormbeta_RL1",
        linesep = "",
        digits = 3,
        caption = "Beta parameters for all-cause mortality, full NHANES III population, normalized by the interquartile range instead of the standard deviation.") %>%
  kable_styling(latex_options = c("striped", "hold_position")))
cat('\n')
```
...
# coxph(Surv(Time+age, eventCVDHrt) ~ age + female + black + other + 
#         M_i_D + M_i_S + D_i_D + D_i_S + 
#         sigma_C_S + sigma_H_S + sigma_C_D + sigma_H_D, 
#       data = DF_nhanesA)
# flexsurv::flexsurvreg(Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
