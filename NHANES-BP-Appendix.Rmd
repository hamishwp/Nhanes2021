...
mean_mat <- rbind(matrix(sub("\\.?0*$","",signif(c(simulation_mean_estimate,true_params,
                    simulation_mean_estimate/true_params-1),4) ), 
                  nrow= 8,ncol = 3,
                  dimnames=list(names(simulation_SE),
                                c("SimAverage",'True','RelError'))),
                  matrix(sub("\\.?0*$","",signif(c(simulation_mean_SE, simulation_SE,
         simulation_mean_SE / simulation_SE -1) ,4) ), nrow= 8,ncol = 3,
           dimnames=list(names(simulation_SE),
                     c("SimAverage",'True SE', 'RelError'))) )
...
kable_combined <- kable(combined_data, format = "latex", escape = FALSE, 
...
```{r iqrnormbeta_RL1, echo=F}
kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%filter(runnum==1)%>%dplyr::select(-runnum)%>%setNames(c("Covariate","Beta Normalised (IQR)","Range")),
format="latex", escape = F,booktabs = T, label = "iqrnormbeta",
caption = "Beta parameters for all-cause mortality, full NHANES III population, normalised by the interquartile range instead of the standard deviation.")

```

```{r iqrnormbeta_RL2, echo=F}
kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%filter(runnum==2)%>%dplyr::select(-runnum)%>%setNames(c("Covariate","Beta Normalised (IQR)","Range")),
format="latex", escape = F,booktabs = T, label = "iqrnormbeta",
caption = "Beta parameters for cardiovascular mortality, full NHANES III population, normalised by the interquartile range instead of the standard deviation.")

```

```{r iqrnormbeta_RL3, echo=F}
kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%filter(runnum==3)%>%dplyr::select(-runnum)%>%setNames(c("Covariate","Beta Normalised (IQR)","Range")),
format="latex", escape = F,booktabs = T, label = "iqrnormbeta",
caption = "Beta parameters for all-cause mortality, FRS NHANES III population, normalised by the interquartile range instead of the standard deviation.")

```

```{r iqrnormbeta_RL4, echo=F}
kable(read_csv(file="Results/beta_all_IQRnorm.csv",show_col_types = FALSE)%>%filter(runnum==4)%>%dplyr::select(-runnum)%>%setNames(c("Covariate","Beta Normalised (IQR)","Range")),
format="latex", escape = F,booktabs = T, label = "iqrnormbeta",
caption = "Beta parameters for cardiovascular mortality, FRS NHANES III population, normalised by the interquartile range instead of the standard deviation.")
...
```{r Mean-SD, echo=FALSE,message=FALSE,warning=FALSE}
...
![ROC curves for the mean systolic and diastolic model, looking non-CVD and heart attack-related deaths, stratified by age group and the number of years the survival outcome was predicted since participant starting the survey. The colour of the points and lines represents the different linear predictor covariate models possible. ](./Rmarkdown_Plots/ROC_CAx-Covariates_EventType_RL2oth.png){#fig:ROC_RL2oth}

![ROC curves for for the FRS-based model, looking specifically at CVD and heart attack-related deaths, stratified by age group and the number of years the survival outcome was predicted since participant starting the survey. The colour of the points and lines represents the different linear predictor covariate models possible. ](./Rmarkdown_Plots/ROC_CAx-Covariates_EventType_RL7.png){#fig:ROC_RL7}
...
![ROC curves for for the FRS-based model, looking non-CVD and heart attack-related deaths, stratified by age group and the number of years the survival outcome was predicted since participant starting the survey. The colour of the points and lines represents the different linear predictor covariate models possible. ](./Rmarkdown_Plots/ROC_CAx-Covariates_EventType_RL8oth.png){#fig:ROC_RL8oth}
...
for (ageB in c(45)){
  for (ty in unique(nhanes$type)){
      fit<-nhanes%>%filter(age<ageB+19 & age>=ageB & 
                                type==ty)%>%
        survival::survfit(formula = survival::Surv(yrsfu, eventhrt) ~ excluded)
...
tmp<-readRDS("./Data_cleaned/nhanes_DF.RData")
DF_nhanesA<-tmp$DF_nhanesA
DF_nhanesFRS<-tmp$DF_nhanesFRS
...
SD_nA <- suppressWarnings(DF_nhanesA %>%
  ...
  setNames(c("term","std")))%>%rbind(data.frame(term=c("race","female"),std=c(1,1)))
...
```{r freqCVDF, echo=F}
# survival::coxph(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other + 
#         M_i_D + M_i_S + D_i_D + D_i_S + 
#         sigma_C_S + sigma_H_S + sigma_C_D + sigma_H_D, 
#       data = DF_nhanesA)
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
survival::coxph(survival::Surv(age, Time+age, eventCVDHrt) ~ FRS.1998 +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesFRS) %>%
  ...
  kableExtra::kable(format = "latex", booktabs = TRUE, label = "freqCVDF",
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
survival::coxph(survival::Surv(age, Time+age, eventall) ~ FRS.1998 +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesFRS) %>%
  broom::tidy(conf.int = TRUE) %>%    # Include confidence intervals
  ...
  setNames(c("Covariate", "Beta Normalised", "P-Value", "Range")) %>%
  mutate(Covariate=gsub("_"," ",Covariate))%>%
  kableExtra::kable(format = "latex", booktabs = TRUE, label = "freqALLF",
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
survival::coxph(survival::Surv(age, Time+age, eventCVDHrt) ~ M_i_S + M_i_D +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesA) %>%
  broom::tidy(conf.int = TRUE) %>%    # Include confidence intervals
  mutate_if(is.numeric, signif, digits = 2) %>%
  # mutate_at(vars(matches("_i_")), scale, scale=F) %>%
  # mutate_at(vars(matches("sigma_")), scale, scale=F) %>%
  ...
  setNames(c("Covariate", "Beta Normalised", "P-Value", "Range")) %>%
  mutate(Covariate=gsub("_"," ",Covariate))%>%
  kableExtra::kable(format = "latex", booktabs = TRUE, label = "freqCVDNF",
...
# flexsurv::flexsurvreg(survival::Surv(Time+age, eventCVDHrt) ~ age + female + black + other, 
#                               data = DF_nhanesA, dist = "gompertz") %>% 
survival::coxph(survival::Surv(age, Time+age, eventall) ~ M_i_S + M_i_D +
                     D_i_S + D_i_D + sigma_C_S + sigma_H_S +
                     sigma_C_D + sigma_H_D +
                  survival::strata(race) + survival::strata(female), data = DF_nhanesA) %>%
  broom::tidy(conf.int = TRUE) %>%    # Include confidence intervals
  ...
  setNames(c("Covariate", "Beta Normalised", "P-Value", "Range")) %>%
  mutate(Covariate=gsub("_"," ",Covariate))%>%
  kableExtra::kable(format = "latex", booktabs = TRUE, label = "freqALLNF",
